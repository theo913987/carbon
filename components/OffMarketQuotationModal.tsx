...
interface OffMarketQuotationModalProps {
  title: string;
  onClose: () => void;
  onSubmit: () => void;
}

export const OffMarketQuotationModal: React.FC<OffMarketQuotationModalProps> = ({ title, onClose, onSubmit }) => {
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <h3 className="text-lg font-bold text-slate-800 truncate pr-4" title={`场外报价 : ${title}`}>
            场外报价 : {title}
          </h3>
        ...
        <div className="p-8 space-y-6">
           <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                 ...
                 <select className="w-full p-2.5 border border-slate-300 rounded-lg text-sm bg-white">
                    <option>全国碳排放权交易中心、北京环境交易所、上海环境能源交易所</option>
                 </select>
              </div>
           </div>
        </div>
